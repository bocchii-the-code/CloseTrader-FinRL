...
### (Recommended) Set up with uv (Python 3.11)

This repo is uv-friendly via `pyproject.toml` + `uv.lock`.
...
# (optional) ensure Python 3.11 is available, then pin
uv python pin 3.11

# create/update the venv from the lockfile
uv sync --locked
...
```bash
# ElegantRL adapter (may pull native deps via gym[box2d])
